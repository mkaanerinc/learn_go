...
type TaxIncludedPriceJob struct {
	...
	TaxRate           float64             `json:"tax_rate"`            // Tax rate to apply (e.g., 0.18 for 18%)
	InputPrices       []float64           `json:"input_prices"`        // Raw input prices before tax
	TaxIncludedPrices map[string]string   `json:"tax_included_prices"` // Optional: stores tax-included prices (currently unused)
}
...
func (job *TaxIncludedPriceJob) Process(doneChan chan bool, errorChan chan error) {
	err := job.LoadData() // Load prices from file
